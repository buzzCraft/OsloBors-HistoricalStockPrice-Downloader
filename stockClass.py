...
class Stock():
    ...
    def xlsxToCsv(self):  
        ...
        read_file = pd.read_excel(xfile)                                       #Read xlsx file
                                                                               #Rename columns
        read_file.rename(columns={f'{self.TICKER}': 'Date', 'Siste': f'{self.TICKER} Last', 'Kjøper': 'Buy', 'Selger' : 'Sell', 'Høy' : 'High', 'Lav' : 'Low', 'Totalt omsatt(NOK)' : 'Total traded(NOK)', 'Totalt antall aksjer omsatt' : 'Volume', 'Antall off. handler' : 'Public trades', 'Antall handler totalt' : 'All trades'}, inplace=True)
  
        read_file.to_csv (cfile, index = None, header=True)                    #Save file
